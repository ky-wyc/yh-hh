...
| 阶段文档 | [MVP 规格](MVP规格.md)、[MVP 验收清单](MVP验收清单.md)、[MVP 验收证据矩阵](MVP验收证据矩阵.md)、[MVP 本地完成状态](MVP本地完成状态.md)、[一期产品蓝图](一期产品蓝图.md)、[二期能力池](二期能力池.md)、[运维说明](运维说明.md) |
| 最后更新 | 2026-07-05 |
...
MVP 只把群文本消息标准化为内部事件；图片、表情、文件和富媒体消息暂不进入路由，后续需要多模态理解或文件知识库时再进入二期能力池。
...
后台 UI 风格参考 `Daymychen/art-design-pro`。该项目采用 Vue 3、Vite、TypeScript、Element Plus 等现代后台技术栈，适合作为本项目后台管理端的视觉、布局和交互参考。MVP 不强制引入 Tailwind CSS 或 TanStack Query，后续可按页面复杂度再补充。
...
command_only：只响应命令。
mention_only：响应命令和 @。
active：响应命令、@ 和明显疑问句；MVP 阶段不对普通陈述主动插话。
admin_only：仅管理员可使用。
```
...
- 新群默认不开启主动发言。
- MVP 支持命令、@ 回复，以及 `active` 模式下对明显疑问句的保守主动回答。
- 主动回答需要后台按群开启。
- 主动发言结果必须记录触发原因。
...
MVP 当前使用 `web-admin` 容器内置 nginx 托管后台静态资源，并通过 Docker 内部网络代理访问 `bot-app:8000`。生产环境如果需要公网访问后台，建议在服务器前置 Nginx、Caddy 或云厂商负载均衡，统一处理 HTTPS、访问控制和安全组策略。
...
- 健康检查接口。
- 后台显示连接状态。
- MVP 使用 PostgreSQL 保存核心业务数据，仅预留 pgvector 扩展；一期知识库启用 pgvector，二期需要独立向量库时再加入 qdrant 服务。
...
```text
PostgreSQL 数据库。
Redis 短期状态（缓存、限流、临时锁；不作为核心恢复依赖）。
知识库原文。
向量库数据。
.env 配置模板。
OneBot 登录态。
...
本节是完整产品验收参考，不作为 MVP 完成判定。MVP 的验收以 [MVP 规格](MVP规格.md) 和 [MVP 验收清单](MVP验收清单.md) 为准；一期和二期能力分别以阶段文档中的验收项为准。
...
+ PostgreSQL
+ Redis
+ MVP 预留 pgvector，一期启用 pgvector，二期按规模选择 Qdrant
+ Docker Compose
